...
- **Angular 18 Vitest Test Environment Initialization**: When running Angular 18 unit tests in Vitest and JSDOM with `@analogjs/vite-plugin-angular`, components require explicit `standalone: true` in component decorators (as standalone did not become default until Angular 19), test initialization requires importing `@angular/compiler` before calling `TestBed.initTestEnvironment(BrowserDynamicTestingModule, platformBrowserDynamicTesting())`, and JSDOM requires polyfilling `window.URL.createObjectURL` and `window.URL.revokeObjectURL` for binary/blob download assertions.
