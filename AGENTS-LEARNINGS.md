...
- **Angular Material MDC Button Label Flex Alignment**: Applying `display: flex` directly to the host `<button mat-flat-button>` (or other MDC button hosts) treats internal MDC structural elements (`.mat-mdc-button-persistent-ripple`, `.mat-focus-indicator`, `.mat-mdc-button-touch-target`, and `.mdc-button__label`) as flex items, distorting layout while leaving nested icons and text unaligned; styling `.mat-mdc-button-base .mdc-button__label` with `display: inline-flex; align-items: center; justify-content: center; gap: 8px;` reliably aligns icons, spinners, and button text inline and vertically centered across all Material buttons.
...
- **Server-Sent Events Progress Replay and Client Fallback**: In asynchronous SSE streaming architectures, clients may establish the EventSource connection after fast backend jobs have already emitted progress or completion events; caching the latest progress state per task ID on the server and replaying it immediately upon new subscriptions, coupled with client-side polling fallbacks, prevents progress UI stalls caused by connection timing race conditions.
