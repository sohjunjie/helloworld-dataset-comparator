...
- **Angular 18 Vitest Test Environment Initialization**: When running Angular 18 unit tests in Vitest and JSDOM with `@analogjs/vite-plugin-angular` and `@analogjs/vitest-angular` (v2.x), import `@analogjs/vitest-angular/setup-zone` and `@angular/compiler` before initializing `TestBed.initTestEnvironment(BrowserDynamicTestingModule, platformBrowserDynamicTesting())`, pin exact versions alongside Vite 6 and Vitest 2 to prevent undeclared peer conflicts, and polyfill `window.URL.createObjectURL` and `window.URL.revokeObjectURL` in JSDOM for binary/blob download assertions.
