...
- **SQL-Based Schema Unification and Tolerant Field Comparison**: When performing cross-dataset diffing across mismatched schemas in columnar engines (such as DuckDB or Parquet), create unified view projections injecting `NULL AS "col"` for missing columns and compare numeric fields using `TRY_CAST(col AS DOUBLE)` with bidirectional boundary expressions (`ABS(v1 - v2) <= pct * ABS(v1) OR ABS(v1 - v2) <= pct * ABS(v2)`) to achieve resilient fallback for mixed-type and disparate-schema datasets. Always cast operands to string (`CAST(col AS VARCHAR) IS NOT DISTINCT FROM CAST(col AS VARCHAR)` or `LOWER(CAST(col AS VARCHAR))`) for join keys and non-tolerance columns to prevent implicit numeric type-coercion errors on mixed-type data.
...
- **Quote-Aware Delimiter Detection & Columnar CSV Ingestion**: When detecting delimiters and converting delimited text files to columnar formats (such as Parquet via DuckDB), delimiter frequency scanning across sample rows must be RFC 4180 quote-aware (ignoring embedded candidate delimiters and escaped double quotes `""` within quoted fields) and the ingestion parser should explicitly configure quote/escape options (`quote='"'`, `escape='"'`) to prevent unintentional column splitting on quoted data.
