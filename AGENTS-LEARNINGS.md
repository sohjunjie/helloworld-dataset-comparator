...
- **Asynchronous Task Timeout Isolation with `CompletableFuture.orTimeout`**: When enforcing timeout boundaries on asynchronous execution pipelines, use `CompletableFuture.orTimeout(timeout, unit)` rather than blocking `future.get(timeout, unit)` on the same thread pool, preventing ForkJoinPool's cooperative `helpAsyncBlocker` mechanism from running delayed tasks inline and bypassing the timeout window in unit test environments.
